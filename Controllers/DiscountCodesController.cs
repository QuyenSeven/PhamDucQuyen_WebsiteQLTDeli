...
namespace QLyNhaHangTDeli.Controllers
{
    public class DiscountCodesController : Controller
    {
        ...
        private string GenerateCodeID(int length)
        {
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (string.IsNullOrEmpty(discountCode.DiscountCodeID))
                    {
                        ...
                        discountCode.DiscountCodeID = GenerateCodeID(6); // Tạo mã ngẫu nhiên với độ dài 8 ký tự
        ...
        [HttpGet]
        public JsonResult GetDiscountById(string id)
        {
            var item = db.DiscountCodes
                .Where(x => x.DiscountCodeID == id)
                .Select(x => new {
                    x.DiscountCodeID,
                    x.Code,
                    x.DiscountType,
        ...
        public ActionResult ListCouponsPartial()
        {
            ...
            var privateLinkedCoupons = db.DiscountCodes
                                .Where(dc => dc.IsPublic == false
                                          && dc.IsActive == true
                                          && dc.CustomerDiscountCodes.Any(c => c.CustomerID == customerId))
                                .ToList();
        ...
        public JsonResult AssignToCustomer(string customerId, string code)
        {
            ...
            {
                CustomerDiscountID = GenerateCodeID(6),
                CustomerID = customerId,
